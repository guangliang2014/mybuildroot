...
/*
PCI: -----pci_init_device:0x8086,0x1237. PCI_DEVICE_ID_INTEL_82441, i440fx_mem_addr_setup
PCI: -----pci_init_device:0x8086,0x7000. PCI_DEVICE_ID_INTEL_82371SB_0  piix_isa_bridge_setup
PCI: -----pci_init_device:0x8086,0x7010. PCI_DEVICE_ID_INTEL_82371SB_1  piix_ide_setup
PCI: -----pci_init_device:0x8086,0x7113. PCI_DEVICE_ID_INTEL_82371AB_3, piix4_pm_setup
*/
...
static void pci_bios_init_device(struct pci_device *pci)
{
    ...
    dprintf(1, "PCI: pci_init_device init pci_device_tbl for %pP\n", pci);
...
static void mch_mem_addr_setup(struct pci_device *dev, void *arg)
{
    ...
    dprintf(1, "PCI: mch_mem_addr_setup called for %pP\n", dev);
...
static void pci_bios_init_platform(void)
{
    struct pci_device *pci;
    foreachpci(pci) {
        dprintf(1, "PCI: pci_init_device init pci_platform_tbl for %pP\n", pci);
...
pci_bios_init_bus_rec(int bus, u8 *pci_bus)
{
    ...
    foreachbdf(bdf, bus) {
        ...
        if (class == PCI_CLASS_BRIDGE_PCI) {
            dprintf(1, "PCI: %s bus = 0x%x, config bdf bridge:0x%x\n", __func__, bus, bdf);
            pci_config_writeb(bdf, PCI_SECONDARY_BUS, 255);
            pci_config_writeb(bdf, PCI_SUBORDINATE_BUS, 0);
        }else{
            dprintf(1, "PCI: %s bus = 0x%x, config bdf not bridge:0x%x\n", __func__, bus, bdf);
        }
    }
    ...
    foreachbdf(bdf, bus) {
        class = pci_config_readw(bdf, PCI_CLASS_DEVICE);
        if (class != PCI_CLASS_BRIDGE_PCI) {
            dprintf(1, "PCI: %s bus = 0x%x, class is not bridge\n", __func__, bus);
            continue;
        }
                        ...
                        int bar, u64 size, u64 align, int type, int is64)
{
    ...
    dprintf(1,"PCI: create region entry: dev=%pP bar=%d size=0x%llx align=0x%llx "
            "type=%s is64=%d\n", dev, bar, size, align,
            region_type_name[type], is64);
    return entry;
}
...
pci_setup(void)
{
    ...
    dprintf(1, "=== PCI pci_bios_init_devices pass #3 ===\n");
    pci_bios_init_devices();    
    dprintf(1, "=== PCI to do free busses ===\n");
    free(busses);
    dprintf(1, "=== PCI done free busses ===\n");
